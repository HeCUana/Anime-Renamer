...
class AnimeOrganizer:
    ...
    def get_season_episodes(self, anime_id, season_number):
        """获取指定季的所有集信息"""
        season_url = f"https://api.themoviedb.org/3/tv/{anime_id}/season/{season_number}"
        params = {
            "api_key": self.api_key,
            "language": "zh-CN"
        }
    ...
    def organize_anime_files(self, folder_path, selected_anime_id):
        ...
        for season in seasons:
            season_number = season['season_number']
            episodes = self.get_season_episodes(selected_anime_id, season_number)
            season_episodes[season_number] = {episode['episode_number']: episode for episode in episodes}
        ...
        for file_path in Path(folder_path).glob('*'):
            if file_path.is_file():
                ...
                for season in seasons:
                    season_number = season['season_number']
                    if season_number in season_episodes and episode_number in season_episodes[season_number]:
                        target_season = season
                        episode_info = season_episodes[season_number][episode_number]
                        break
